...
- 现有 H5 已经是 `/app/ai/chat` SSE 和预约接口的客户端，本阶段不修改 H5。
...
- 不依赖 H5 也能通过后端 API、SSE 客户端和自动化测试完成上述闭环。
...
- 不重构已有 Admin 知识管理页面。
- 本阶段不复制、修改或构建 H5 前端；H5 集成作为后续阶段。
...
| 模块 | 负责什么 | 不负责什么 |
| --- | --- | --- |
| H5/外部客户端 | 未来消费 SSE 和预约 API | 本阶段不修改；不解析模型工具调用，不直接写业务表 |
...
H5 或其它客户端不依赖模型原始响应格式，只依赖这些稳定事件。本阶段使用 API/SSE 测试客户端验证，不要求前端参与。
...
H5 本阶段不纳入开发和验收。后续只需要按照本规格消费稳定的 SSE、草稿、确认和预约查询接口，不应重新实现 Agent、RAG 或预约业务逻辑。
...
11. 后端单元测试、迁移安全测试、API/SSE 闭环测试和 RAG 评测有可复现命令；H5 不属于本阶段验收。
