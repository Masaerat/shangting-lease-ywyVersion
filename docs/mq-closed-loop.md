...
> 面向学习和面试的完整解释见 [升级改造全解与面试手册](agent-rag-mq-interview-handbook.md)；本文聚焦部署、状态与故障重放。
...
```powershell
docker run --rm -v "${PWD}:/workspace" -w /workspace/backend maven:3.9.9-eclipse-temurin-21 mvn -B -ntp -pl web-app,web-admin -am '-Dtest=*Test,!ScheduledTasksTest' '-Dsurefire.failIfNoSpecifiedTests=false' test
```
