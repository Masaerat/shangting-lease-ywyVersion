...
> 适用分支：`master`
> 对应代码范围：已合并 Agent/RAG/MQ、分层记忆与 RAG 评测升级的后端最终结构
...
目录整理后，根目录只保留 `backend/`、`deploy/`、`docs/` 三个主目录及 Compose 等入口配置。上图四个模块由 `backend/pom.xml` 直接聚合，删除了无独立业务代码的中间 `web` 聚合层。`model` 仍是共享模块，不能归入 `web-app`：后台也要使用房源、用户和预约等实体，依赖关系不应倒置为“后台依赖用户端应用”。构建统一使用固定 Maven 3.9.9 与 JDK 21 的 Docker 镜像；对象存储、PGvector 初始化和验收脚本集中在 `deploy/`。当前仓库不再包含 H5，旧前端可从 Git 提交 `a20894b` 恢复。
...
评测集位于 `backend/web-app/src/test/resources/ai/rag-evaluation-dataset.jsonl`。V1 共 30 条：24 条可回答问题、6 条无答案问题，覆盖押金、付款、预约、报修、退租和入住材料，并给同一意图准备多种口语表达。
...
`febf895` 的 MQ 闭环提交前曾执行 31 个测试类、87 项测试并全部通过。加入分层记忆和 RAG 评测后，在目录重构前使用 Docker Maven 镜像完成以下回归；目录重构后的结果单独记录在[最终验证记录](verification.md)：
...
- 当时的父工程及子模块编译成功。当前目录取消中间 `web` 聚合层，Maven Reactor 为父工程与四个子模块，共五个项目；从仓库根目录执行编译使用 `mvn -f backend/pom.xml -B -DskipTests compile`。
...
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/agent/DefaultRentalAgentRuntime.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/agent/ToolRegistry.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/rag/HybridRentalKnowledgeService.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/rag/evaluation/RagRetrievalEvaluator.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/memory/ConversationMemoryService.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/controller/ai/AiRagController.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/appointment/AppointmentDraftService.java`
- `backend/web-app/src/main/java/com/atguigu/lease/web/app/service/ai/appointment/AppointmentConfirmationService.java`
- `backend/common/src/main/java/com/atguigu/lease/outbox/AppointmentOutboxRepository.java`
- `backend/common/src/main/java/com/atguigu/lease/outbox/RabbitAppointmentEventSender.java`
- `backend/common/src/main/java/com/atguigu/lease/consumer/appointment/AppointmentMessageConsumer.java`
- `backend/common/src/main/java/com/atguigu/lease/notification/AppointmentNotificationStore.java`
- `backend/web-app/src/main/resources/db/migration/V5__user_notification.sql`
