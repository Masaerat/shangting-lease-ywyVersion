...
功能迭代日期：2026-09-06；原开发分支：`agentRag`，已合入 `master`。源码路径已按当前目录结构更新。
...
```text
backend/web-app/src/test/resources/ai/rag-evaluation-dataset.jsonl
```
...
以下是功能迭代阶段在 `agentRag` 工作区中使用 `maven:3.9.9-eclipse-temurin-21` Docker 镜像取得的历史结果，不依赖宿主机安装 JDK。目录重构后的回归结果见[最终验证记录](verification.md)：

- 新增能力定向测试：22 项，失败 0、错误 0、跳过 0。
- `web-app` 全量非集成测试：92 项，失败 0、错误 0、跳过 0。
- `web-admin` 知识切块、文档入库和房源入库测试：4 项，失败 0、错误 0、跳过 0。
- 当时父工程和全部子模块编译成功；当前 `backend/pom.xml` 直接聚合 `model`、`common`、`web-admin`、`web-app`，已删除中间 `web` 聚合层。
