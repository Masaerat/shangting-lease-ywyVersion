...
> 如果目标是系统学习整个项目升级过程并准备面试，请优先阅读 [升级改造全解与面试手册](agent-rag-mq-interview-handbook.md)。本文保留为 Agent Harness 专题材料。

> 2026-09-06 增量：工具已由五个扩展为七个，新增预约状态/站内通知查询；消费链路改为幂等落库，回退共用预算并保留部分工具结果。以下为此前基线，更新内容以 [MQ 闭环说明](mq-closed-loop.md) 为准。
